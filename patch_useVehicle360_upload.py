...
old_upload = """  const uploadFrames = async (files: File[]) => {
    if (!project) return;
    try {
      setUploading(true);
      setUploadProgress({ current: 0, total: files.length });

      // First clear existing frames if any (simplified: we should warn user before calling this)
      ...
      // Upload sequentially to keep it simple and reliable
      let successCount = 0;
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const path = `360/${project.id}/${Date.now()}_frame_${i}.jpg`;
