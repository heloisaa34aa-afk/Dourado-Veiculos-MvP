...
import { 
  ...
  X, Info, Edit2, Move, Focus, Eye, EyeOff, Save, Play, ArrowLeft, Loader2, Camera, MousePointer2
, Car as CarIcon } from 'lucide-react';
...
function Vehicle360Workspace({ vehicleId, car, onBack }: { vehicleId: string, car: any, onBack: () => void }) {
  const { 
  ...
  if (loading || !project) {
    return <div className="flex items-center justify-center h-screen bg-gray-50"><Loader2 className="animate-spin text-indigo-500 w-12 h-12" /></div>;
  }
    ...
    worker.onmessage = (e) => {
      ...
      if (type === 'progress') {
        ...
        const finalResults = results.map((r: any) => ({
           ...r,
           isKeyframe: r.frameNumber === initialFrame
        }));
        setTrackedPositions(finalResults);
        setMode('review');
        worker.terminate();
      } else if (type === 'error') {
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="p-4 border-b border-gray-200 space-y-2 bg-gray-50/50">
          <button onClick={() => setMode('add_poi_pick')} className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 shadow-sm transition-transform active:scale-95">
            <Plus size={18} /> Adicionar Ponto de Interesse
          </button>
          <button onClick={() => setMode('add_damage_pick')} className="w-full flex items-center justify-center gap-2 py-2.5 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 shadow-sm transition-transform active:scale-95">
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-100">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
         <div className="flex-1 flex flex-col min-w-0 bg-gray-900">
            <div className="flex-1 relative flex items-center justify-center p-2 sm:p-8">
               ...
               {(mode === 'add_poi_pick' || mode === 'add_damage_pick') && (
                 <div className="absolute top-8 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur text-indigo-900 px-6 py-3 rounded-full font-bold shadow-2xl flex items-center gap-3 animate-pulse border border-indigo-100">
            ...
            {!missingFrames && (
              <div className="h-24 bg-gray-950 border-t border-gray-800 overflow-x-auto flex items-center px-4 gap-2 shrink-0 scrollbar-thin scrollbar-thumb-gray-700">
                 {project.frames?.map((f, idx) => {
                   ...
                   return (
                     <button 
                       ...
                       className={`relative h-16 aspect-video shrink-0 rounded-md overflow-hidden border-2 transition-all ${idx === currentFrame ? 'ring-2 ring-white scale-105 z-10' : 'hover:border-gray-500'} ${frameClasses}`}
                     >
                       <img src={f.imageUrl} alt="" className="w-full h-full object-cover" />
                       <div className="absolute top-0 left-0 bg-black/60 text-white text-[10px] px-1.5 py-0.5 rounded-br font-mono">{idx + 1}</div>
                       {indicator}
                     </button>
                   );
                 })}
         ...
         <div className="w-full md:w-96 bg-white border-l border-gray-200 flex flex-col shadow-2xl z-20 shrink-0">
             {renderRightPanel()}
         </div>
      </div>
    </div>
